...
template <class Sample> class SignalProcessor{
	public:
		...
		void setPitches(std::vector<float>);
	...
	overlapFactor(overlapFactor), log2FftSize(log2FftSize)
{
	...
	outputPhase=new Sample[(1<<(log2FftSize-1))+1];
	...
	for(int i=0; i<(1<<(log2FftSize-1))+1; ++i) outputPhase[i]=0;
	//loops
	samplesPerBeat=0;
	beatsPerMeasure=0;
	loopToRecord=-1;
...
template <class Sample> void SignalProcessor<Sample>::setPitches(std::vector<float> p){
	pitches=p;
}
...
template <class Sample> void SignalProcessor<Sample>::pitchShift(
	...
	if(outputIndex==samplesPerFft){//if it's time to do an stft
		...
		for(int k=0; k<=(1<<(log2FftSize-1)); ++k){
			...
			dPhase-=2*PI*int((dPhase+PI)/(2*PI));
		...
		for(int i=0; i<(1<<(log2FftSize-1)); ++i) outputFrequency[i]=0;
		for(unsigned i=0; i<pitches.size(); ++i){
			for(int k=0; k<=(1<<(log2FftSize-1)); ++k){
				int j=int(k*pitches[i]);
				if(j<=(1<<(log2FftSize-1))){
					outputMagnitude[j]+=inputMagnitude[k];
					outputFrequency[j]+=inputFrequency[k]*pitches[i];
				}
			}
		}
		//transform back
		for(int k=0; k<=(1<<(log2FftSize-1)); ++k){
			...
			outputPhase[k]+=dPhase;
			//get real and imaginary parts and reinterleave
			fftSignal[2*k]=magnitude*Sample(cos(outputPhase[k]));
			fftSignal[2*k+1]=magnitude*Sample(sin(outputPhase[k]));
		}
		...
		for(int k=0; k<(1<<log2FftSize); ++k){
			...
			outputHistory[k]=outputHistory[k+samplesPerFft]+2*window*fftSignal[2*k]/((1<<(log2FftSize-1))*overlapFactor);
		}
	}
}
