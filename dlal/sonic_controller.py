...
class Oscillator:
    def __init__(self, i, oscillators, sonic_controller):
        self.sonic_controller = sonic_controller
        column = itertools.count()
        self.label = tkinter.Label(text=str(i)).grid(row=i, column=next(column))
        self.a = tkinter.Scale(command=lambda x: sonic_controller.live_command('a {0:d} {1:f}'.format(i, self.a.get()**self.exponents[self.a])))
        self.a.grid(row=i, column=next(column))
        self.d = tkinter.Scale(command=lambda x: sonic_controller.live_command('d {0:d} {1:f}'.format(i, self.d.get()**self.exponents[self.d])))
        self.d.grid(row=i, column=next(column))
        self.s = tkinter.Scale(command=lambda x: sonic_controller.live_command('s {0:d} {1:f}'.format(i, self.s.get()**self.exponents[self.s])))
        self.s.grid(row=i, column=next(column))
        self.r = tkinter.Scale(command=lambda x: sonic_controller.live_command('r {0:d} {1:f}'.format(i, self.r.get()**self.exponents[self.r])))
        self.r.grid(row=i, column=next(column))
        self.m = tkinter.Scale(command=lambda x: sonic_controller.live_command('m {0:d} {1:f}'.format(i, self.m.get()**self.exponents[self.m])))
        self.m.grid(row=i, column=next(column))
        self.o = tkinter.Scale(command=lambda x: sonic_controller.live_command('o {0:d} {1:f}'.format(i, self.o.get()**self.exponents[self.o])))
        self.o.grid(row=i, column=next(column))
        self.i = [None]*oscillators
        for j in range(oscillators):
            def command(oscillator):
                return lambda x: sonic_controller.live_command(
...
class VgmSetting:
    ...
    def set(self, command, value, channel=None, op=None):
        if op != None:
            self.op_commands[channel][op][command] = value
        elif channel != None:
            self.channel_commands[channel][command] = value
        else:
...
class SonicController(Component):
    ...
    def set_range(self, start=None, end=None, resolution=None, exponent=None, scale=None, default=False):
        ...
        if scale:
            ...
            scales = [s for o in self.oscillators for s in o.scales()]
        for s in scales:
            s.config(from_=end, to=start, resolution=resolution)
            if exponent:
                for o in self.oscillators:
                    o.set_exponent(s, exponent)
        if default:
            for o in self.oscillators:
        ...
        with open(vgm_file_name, 'rb') as vgm_file:
            ...
            while True:
                ...
                if not byte:
                    break
                vgm.append(ord(byte))
        if vgm[0:4] != [ord(x) for x in 'Vgm ']:
            raise Exception('vgm: wrong file identification')
        if le(vgm[0x8:0xc]) != 0x150:
            raise Exception('vgm: unhandled version')
        if samples == None:
            samples = le(vgm[0x18:0x1c])
        sample = start_samples
        if setting == None:
            setting = VgmSetting(sample)
        i = 0x34+le(vgm[0x34:0x38])
        while i < len(vgm) and sample <= samples:
            if vgm[i] in [0x4f, 0x50]:
                i += 2
            elif vgm[i] == 0x51:
                i += 3
            elif vgm[i] in [0x52, 0x53]:
                if sample < start_samples:
                    continue
                ...
                if vgm[i+1] == 0x22:
                ...
                elif vgm[i+1] in [0x24, 0x25, 0x26]:
                    pass  # timers
                elif vgm[i+1] == 0x27:
                    # ignore timer parts
                    ...
                    if vgm[i+2] != 0:
                        raise Exception('vgm: proprietary register not set to 0')
                elif vgm[i+1] in [0xa0, 0xa4, 0xa1, 0xa5]:
                    pass  # frequency of normal channels
                    ...
                    raise Exception('vgm: unhandled register {0:x}'.format(vgm[i+1]))
                i += 3
            elif vgm[i] == 0x54:
                i += 3
            elif vgm[i] == 0x61:
                ...
                raise Exception('vgm: unhandled command 0x{0:x} at offset 0x{1:x}'.format(vgm[i], i))
        return setting
        ...
        for command, value in commands['channel'].items():
            if command == 'feedback':
                if not value:
                    ...
                    feedback = 2.0**(value-9)
            elif command == 'algorithm':
                algorithm = value
        for i in range(len(commands['ops'])):
            for command, value in commands['ops'][i].items():
                if command == 'multiply':
                    self.oscillators[i].m.set(value)
                elif command == 'total level':
                    total_level[i] = 2**(-value/32.0)
                elif command == 'attack rate':
                    self.oscillators[i].a.set(2**((value-1)/2.0)/(sample_rate*8))
                elif command == 'first decay rate':
                    self.oscillators[i].d.set(2**((value-1)/2.0)/(sample_rate*24))
                elif command == 'second level':
                    self.oscillators[i].s.set(1.0/2**value)
                elif command == 'release rate':
                    self.oscillators[i].r.set(2**value/(sample_rate*24.0))
