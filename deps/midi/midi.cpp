...
template<typename T> struct Slice{
	...
	void checkBounds(const std::vector<T>& v, unsigned start=0) const {
		if(start>v.size()) throw std::logic_error("bad slice -- start too large");
		if(start+_size>v.size()) throw std::logic_error("bad slice -- size too large");
	}
	const T* _p;
