...
class SimpleSystem:
    ...
    def __init__(self, components, midi_receivers=None, outputs=None,
        ...
        if self.test:
            self.midi.midi(0x90, test_note, 0x40, immediate=True)
        ...
        # connect
        x = components
        if midi_receivers != None:
            x = midi_receivers
        for component in x:
            self.midi.connect(component, immediate=True)
        x = components
        if outputs != None:
            x = outputs
        for component in x:
            component.connect(self.audio, immediate=True)
